...
void statusInit(vector<int>& potions, int* stat) {
	...
	printf("* You received %d HP Potions and %d MP Potions.\n", RPGConst::Init::HEALTH_POTION,
	       RPGConst::Init::MANA_POTION);
	...
	while (choice > 0 && choice < 6) {
		...
		switch (choice) {
			...
			case 4:
				...
				break;
			case 5:
				printf(RPGConst::Message::InitSystem::CURRENT_STATUS, stat[RPGConst::Status::HEALTH_POINT],
				       stat[RPGConst::Status::MANA_POINT], stat[RPGConst::Status::ATTACK_POINT],
				       stat[RPGConst::Status::DEFENSE_POINT]);
				break;
			case 0:
				printf(RPGConst::Message::InitSystem::START_GAME, stat[RPGConst::Status::HEALTH_POINT],
				       stat[RPGConst::Status::MANA_POINT], stat[RPGConst::Status::ATTACK_POINT],
				       stat[RPGConst::Status::DEFENSE_POINT]);
				break;
			default:
...
bool battileStart(Player* player, vector<int> potions) {
	Slime slime;
	printf(RPGConst::Message::Fight::BATTLE_START, player->getName().c_str(), player->getJob().c_str(),
	       slime.getName().c_str());
	int select = 0, choose;
		...
		while (chooseContinue) {
			...
			if (select == 1) {
				...
				while (true) {
					...
					if (choose == 1) {
						if (potions[0] > 0) {
							int max = player->getHp() + 50 > 100 ? 100 : player->getHp() + 50;
							printf(RPGConst::Message::Fight::HP_POTION_USE, player->getHp(), max);
							player->setHp(max);
							break;
						}
						cout << RPGConst::Message::InitSystem::NOT_ENOUGH_HEALTH_POTION;
						...
						if (potions[1] > 0) {
							int max = player->getMp() + 50 > 100 ? 100 : player->getMp() + 50;
							printf(RPGConst::Message::Fight::MP_POTION_USE, player->getMp(), max);
							player->setMp(max);
							break;
						}
						cout << RPGConst::Message::InitSystem::NOT_ENOUGH_MANA_POTION;
...
int main() {
	...
	while (isGameStart) {
		...
		cin >> select;
		switch (select) {
			case 1: isGameStart = battileStart(player, potions);
				break;
			case 2: player->printInventory();
				break;
			case 3: potionShop(potionRecipe);
				break;
			case 0:
